...
// fucky shit because av_err2str 'is undefined' even though it clearly was defined in the libavutil/opt header, and else it would be in the libavutil/error.h
...
int encode_video(StreamingContext *decoder, StreamingContext *encoder, AVFrame *input_frame)
{
	if (input_frame)
		...
		input_frame->pict_type = AVPictureType(0);
...
int main(int argc, char *argv[])
{
   ...
   */
	...
	if (!sp.copy_video)
	{
		...
		AVRational input_framerate = decoder->video_avs->avg_frame_rate;
